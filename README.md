...
|---|---|---|
| `dotss list` | Displays available categories and contained packages for current platform | `-sh` / `--show-hidden` to display unsupported categories |
...
| `dotss show <category>` | Displays full description, package tracking status, config file actions, and post-install commands | Accepts category name or alias (e.g. `zsh-tokyonight`, `zsh-tn`) |
