...
- **Prerequisite Binaries**: Verifies presence of `curl` and `tar`.
- **Package Presence Check**:
  - **Debian**: `dpkg-query -W -f='${db:Status-Status}' <package>` (or `which <command>`)
  - **Termux**: `dpkg-query -W -f='${db:Status-Status}' <package>` (or `which <command>`)
...
| **Package Check** | `dpkg-query -W` | `dpkg-query -W` / `pkg status` |
...
| **Binary Symlinks** | `~/.local/bin/` | `$PREFIX/bin/` or `~/.local/bin/` |
